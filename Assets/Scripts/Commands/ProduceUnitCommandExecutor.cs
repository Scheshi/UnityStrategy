    ...
    public class ProduceUnitCommandExecutor : CommandExecutorBase<ICreateUnitCommand>, IUnitProducer, ITick
    {
        ...
        public void Tick()
        {
            ...
            if (innerTask.TimeProduce <= 0)
            {
                ...
                unit.GetComponent<ISelectableItem>().SetExecutors(new MoveCommandExecutor(unit.GetComponent<NavMeshAgent>()), new AttackCommandExecutor(unit.GetComponent<IAttacker>(), unit.GetComponent<NavMeshAgent>()), new PatrolCommandExecutor(unit.GetComponent<NavMeshAgent>()));
                RemoveTaskAtIndex(0);
            }
        }
